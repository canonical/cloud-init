...
GOTO="cloudinit_end"
""".format(
    "{ID_NET_DRIVER}", "e1000|e1000e|vlance|vmxnet2|vmxnet3|vrdma"
)


VMW_METADATA_YAML_WITH_NET_DRIVERS_AS_LIST = """instance-id: cloud-vm
local-hostname: cloud-vm
network-drivers:
- vmxnet2
- vmxnet3
...
VMW_METADATA_YAML_WITH_NET_DRIVERS_AS_STRING = """instance-id: cloud-vm
local-hostname: cloud-vm
network-drivers: vmxnet2   | vmxnet3
"""
...
class TestDataSourceVMwareGuestInfo(FilesystemMockingTestCase):
    ...
    @mock.patch("cloudinit.sources.DataSourceVMware.guestinfo_get_value")
    @mock.patch("cloudinit.sources.DataSourceVMware.which")
    def test_extra_hotplug_udev_rules_with_net_drivers_as_list(
        self,
        m_which_fn,
        m_get_fn,
    ):
        ...
        m_get_fn.side_effect = [
            VMW_METADATA_YAML_WITH_NET_DRIVERS_AS_LIST,
            "",
            "",
            "",
            "",
    ...
    @mock.patch("cloudinit.sources.DataSourceVMware.guestinfo_get_value")
    @mock.patch("cloudinit.sources.DataSourceVMware.which")
    def test_extra_hotplug_udev_rules_with_net_drivers_as_string(
        self,
        m_which_fn,
        m_get_fn,
    ):
        m_which_fn.side_effect = ["vmtoolsd", "vmware-rpctool"]
        m_get_fn.side_effect = [
            VMW_METADATA_YAML_WITH_NET_DRIVERS_AS_STRING,
            "",
            "",
            "",
            "",
