...
class TestGetCurrentRenameInfo:
    ...
    receive link-local (169.254.x.x, scope link) IPv4 addresses before
    cloud-init runs init-local. Previously, ip -4 addr show (no scope filter)
    included these, causing downable=False and aborting interface renaming with
    a [busy] error.  The fix adds 'scope global' to mirror IPv6 behavior.
    """
    ...
    def test_link_local_ipv4_does_not_block_downable(
        self, mock_is_up, mock_get_interfaces, mock_subp
    ):
        """Interface with only a 169.254.x.x link-local address must be
        downable. Fixes GH #6887: ip -4 addr show scope global excludes
        scope link addresses assigned by kernel autoconfig / systemd-networkd
        before cloud-init init-local runs."""
        mock_get_interfaces.return_value = [self.IFACE]
        # ip -4 addr show scope global returns nothing -- no global addr
        ...
        assert info["ens18"]["downable"] is True, (
            "Interface with only a link-local IPv4 must be downable "
            "(fix for GH #6887)"
        )
    ...
    @mock.patch("cloudinit.net.get_interfaces")
    @mock.patch("cloudinit.net.is_up", return_value=True)
    def test_global_ipv4_marks_not_downable(
        self, mock_is_up, mock_get_interfaces, mock_subp
    ):
        """Interface with a routable global IPv4 (e.g. DHCP or static)
        ...
        info = net._get_current_rename_info(check_downable=True)
        assert info["ens18"]["downable"] is False, (
            "Interface with a global routable IPv4 must not be downable"
        )

    @mock.patch("cloudinit.subp.subp")
    @mock.patch("cloudinit.net.get_interfaces")
    @mock.patch("cloudinit.net.is_up", return_value=True)
    def test_global_ipv6_marks_not_downable(
        self, mock_is_up, mock_get_interfaces, mock_subp
    ):
        """Interface with a permanent global IPv6 address must NOT be
        downable, consistent with the existing IPv6 scope global filter."""
        mock_get_interfaces.return_value = [self.IFACE]
        ipv6_out = (
    ...
    @mock.patch("cloudinit.net.get_interfaces")
    @mock.patch("cloudinit.net.is_up", return_value=True)
    def test_ipv4_scope_global_flag_is_used(
        self, mock_is_up, mock_get_interfaces, mock_subp
    ):
        """Regression guard: verify the ip -4 command includes 'scope global'.
        Ensures GH #6887 cannot silently regress."""
        mock_get_interfaces.return_value = [self.IFACE]
        mock_subp.return_value = ("", "")
        net._get_current_rename_info(check_downable=True)
        ipv4_calls = [
            c for c in mock_subp.call_args_list if "-4" in str(c)
        ]
        assert len(ipv4_calls) == 1
        cmd = ipv4_calls[0][0][0]
        assert "scope" in cmd and "global" in cmd, (
            f"ip -4 addr show must include 'scope global' but got: {cmd}"
        )
