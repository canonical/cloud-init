...
class TestResolvConf:
    ...
    def call_resolv_conf_handler(self, distro, conf, tmp_path):
        ds = None
    ...
    def test_resolv_conf_systemd_resolved(
        self, m_render_to_file, Distro, tmp_path
    ):
        dist = Distro("photon", self.cfg)
        self.call_resolv_conf_handler(dist, self.cfg, tmp_path)
    ...
    @mock.patch("cloudinit.config.cc_resolv_conf.templater.render_to_file")
    def test_resolv_conf_no_param(self, m_render_to_file, caplog, tmp_path):
        tmp = deepcopy(self.cfg)
        tmp.pop("resolv_conf")
        self.call_resolv_conf_handler("photon", tmp, tmp_path)
    ...
    def test_resolv_conf_manage_resolv_conf_false(
        ...
        dist = Distro("photon", self.cfg)
        self.call_resolv_conf_handler(dist, tmp, tmp_path)
