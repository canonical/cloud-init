...
def init_class(clean_paths):
    ...
    init.distro.ci_sudoers_fn = clean_paths.tmpdir.join(
...
class TestClean:
    ...
    def test_remove_sudoers_conf(self, clean_paths, init_class):
        """remove_config removes the cloud-init sudoers file."""
        sudoers_path = clean_paths.tmpdir.join(
            "etc/sudoers.d/90-cloud-init-users"
        )
        ensure_dir(os.path.dirname(sudoers_path))
        sudoers_path.write(
            "# Created by cloud-init\ntest ALL=(ALL) NOPASSWD:ALL\n"
        )
        init_class.distro.ci_sudoers_fn = sudoers_path.strpath

        retcode = clean.remove_artifacts(
            init_class,
            remove_logs=False,
            remove_config=["sudoers"],
        )

        assert (
            sudoers_path.exists() is False
        ), f"Unexpected file {sudoers_path}"
        assert 0 == retcode
    ...
    def test_remove_all_conf(self, clean_paths, init_class):
        ...
        assert sudoers_path.exists() is False, f"Unexpected {sudoers_path}"
        ensure_dir(os.path.dirname(sudoers_path))
        sudoers_path.write("# Created by cloud-init\n")
        init_class.distro.ci_sudoers_fn = sudoers_path.strpath
        ...
        for conf_path in TEST_GEN_SSH_CONFIG_FILES:
            assert conf_path.exists() is False, f"file {conf_path} exists!"
        assert sudoers_path.exists() is False, f"file {sudoers_path} exists!"
        assert 0 == retcode
