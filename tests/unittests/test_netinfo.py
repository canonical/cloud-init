...
class TestGetHostIP:
    """Tests for get_host_ip IPv4/IPv6 address resolution."""
    ...
    def test_get_host_ip(self, mocker, netdev_response, expected_ip):
        ...
        if isinstance(netdev_response, Exception):
            mocker.patch(
                "cloudinit.netinfo.netdev_info", side_effect=netdev_response
            )
        else:
            mocker.patch(
                "cloudinit.netinfo.netdev_info", return_value=netdev_response
            )
        assert get_host_ip() == expected_ip
