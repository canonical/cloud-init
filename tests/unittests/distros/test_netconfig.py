...
def _get_distro(dname, renderers=None, activators=None):
...
def distro_freebsd(mocker):
    with open("tests/data/netinfo/freebsd-ifconfig-output", "r") as fh:
        ...
        return _get_distro("freebsd", renderers=["freebsd"])
...
@pytest.fixture
def distro_eni():
    return _get_distro("ubuntu", renderers=["eni"], activators=["eni"])
...
class TestNetCfgDistroUbuntuEni:
    ...
    def _apply_and_verify_eni(
        ...
        results = dir2dict(
            str(tmpd), filter=lambda fn: fn.startswith(str(tmpd / "etc"))
        )
        for cfgpath, expected in expected_cfgs.items():
            print("----------")
            print(expected)
    ...
    def test_apply_network_config_and_bringup_activator_defaults_ub(
        ...
        distro = _get_distro("ubuntu", renderers=["eni"])
        with mock.patch(
            "cloudinit.net.activators.select_activator"
        ) as select_activator:
            select_activator.return_value = IfUpDownActivator
...
@pytest.fixture
def distro_netplan():
    return _get_distro("ubuntu", renderers=["netplan"])
...
class TestNetCfgDistroUbuntuNetplan:
    ...
    def _apply_and_verify_netplan(
        ...
        for cfgpath, expected, mode in expected_cfgs:
            ...
            print("----------")
            assert expected == results[cfgpath]
            assert mode == get_mode(cfgpath, str(tmpd))
...
@pytest.fixture
def distro_redhat():
    return _get_distro("rhel", renderers=["sysconfig"])
...
class TestNetCfgDistroRedhat:
    ...
    def _apply_and_verify(
        ...
        for cfgpath, expected in expected_cfgs.items():
            ...
            assert 0o644 == get_mode(cfgpath, str(tmpd))
...
@pytest.fixture
def distro_opensuse():
    return _get_distro("opensuse", renderers=["sysconfig"])
...
@pytest.fixture
def distro_arch():
    return _get_distro("arch", renderers=["netplan"])
...
def distro_photon(mocker):
    ...
    return _get_distro("photon", renderers=["networkd"])
...
def distro_mariner(mocker):
    mocker.patch("cloudinit.net.networkd.util.chownbyname")
    return _get_distro("mariner", renderers=["networkd"])
...
def distro_azurelinux(mocker):
    mocker.patch("cloudinit.net.networkd.util.chownbyname")
    return _get_distro("azurelinux", renderers=["networkd"])
