...
def convert_to_network_config_v2(config):
    """
        Convert the UpCloud network metadata description into
        Cloud-init's version 2 netconfig format.
    ...
    for raw_iface in config.get("interfaces"):
        ...
        interface = {"match": {"macaddress": mac_address}}

        iface_dns = config.get("dns")
        dhcp_configured = False

        for ip_addr in raw_iface.get("ip_addresses"):
            if ip_addr.get("dhcp"):
            ...
            if "/" not in ip_addr.get("network"):
                LOG.error(
                    "CIDR notation missing from network, not adding %s",
                    ip_addr.get("address"),
                )
                continue
            static_address = "{}/{}".format(
                ip_addr.get("address"), ip_addr.get("network").split("/")[1]
            )

            if "addresses" not in interface:
                interface["addresses"] = []
            interface["addresses"].append(static_address)

            if ip_addr.get("gateway"):
                interface[
                    "gateway{}".format(6 if "family" == "IPv6" else 4)
                ] = ip_addr.get("gateway")

            if ip_addr.get("dns") and\
                    (ip_addr.get("dns") != config.get("dns")):
                interface["nameservers"] = ip_addr.get("dns")

        if not dhcp_configured:
            interface["nameservers"] = {"addresses": iface_dns}

        LOG.debug(
            "Generated interface config for %s: %s",
            sysfs_name, interface
        )
        nic_configs[sysfs_name] = interface

    return {"version": 2, "ethernets": nic_configs}
...
def convert_to_network_config_v1(config):
    ...
    Example JSON can be seen above in v2 conversion function.
    """
...
def convert_network_config(config, version=2):
    if version == 2:
        return convert_to_network_config_v2(config)
