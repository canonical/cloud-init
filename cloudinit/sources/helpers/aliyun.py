...
def get_instance_meta_data(
    ...
    meta_data_raw = util.load_json(response.contents)

    # The `meta-data/all` URL returns a JSON object with the following format:
    # { ... "nameservers": "100.100.2.136\n100.100.2.138"  ...}
    # Note: The value of the `nameservers` key is a string,
    # This format is the same as the response from the
    # `meta-data/dns-conf/nameservers` endpoint. we use the same serialization
    # method to ensure consistency between
