...
def partition_type_matches(found_type, expected_type):
    ...
    if len(found_type) == 4:
        if found_type in sgdisk_to_gpt_id:
            ...
            raise RuntimeError("Cannot find GPT GUID for found type %s" %
                               found_type)
    if len(expected_type) == 4:
        if expected_type in sgdisk_to_gpt_id:
            expected_type = sgdisk_to_gpt_id[expected_type]
        else:
            raise RuntimeError("Cannot find GPT GUID for expected type %s" %
                               found_type)
    ...
    for partition_type, (start, end) in layout:
        ...
        if len(partition_type) != 36:
            if partition_type != 'None':
                LOG.warning("Unknown GPT partition type %s, using Linux",
                            partition_type)
